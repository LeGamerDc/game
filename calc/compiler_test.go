...
func TestCompile(t *testing.T) {
	...
	t.Run("运算符优先级", func(t *testing.T) {
		// 测试幂运算优先级
		compiledFunc, err := Compile[*MockKv]("int x, y, z; x + y ^ z")
		assert.Nil(t, err)
		...
		// 测试复杂的混合运算
		compiledFunc2, err := Compile[*MockKv]("int a, b, c, d; a * b + c * d")
		assert.Nil(t, err)
	...
	t.Run("嵌套三目运算符", func(t *testing.T) {
		compiledFunc, err := Compile[*MockKv]("int x, y, z; x > 0 ? (y > 0 ? x + y : x - y) : (z > 0 ? z : 0)")
		assert.Nil(t, err)
	...
	t.Run("复杂逻辑表达式", func(t *testing.T) {
		compiledFunc, err := Compile[*MockKv]("int x, y, z; float a, b; (x > 5 && y < 10) || (a >= 3.14 && b <= 2.71)")
		assert.Nil(t, err)
	...
	t.Run("多重赋值和复杂表达式", func(t *testing.T) {
		compiledFunc, err := Compile[*MockKv]("int x, y, z, result; float ratio; x = 10; y = 5; z = x * y; ratio = z / 25.0; result = ratio > 1.5 ? z + 10 : z - 10")
		assert.Nil(t, err)
	...
	t.Run("浮点数精度处理", func(t *testing.T) {
		compiledFunc, err := Compile[*MockKv]("float x, y, z; x = 0.1; y = 0.2; z = x + y; z > 0.29 && z < 0.31")
		assert.Nil(t, err)
		...
		// 测试更复杂的浮点数运算
		compiledFunc2, err := Compile[*MockKv]("float a, b, c; a = 3.14159; b = 2.71828; c = a ^ 2 + b ^ 2; c > 15 && c < 18")
		assert.Nil(t, err)
	...
	t.Run("类型混合运算", func(t *testing.T) {
		compiledFunc, err := Compile[*MockKv]("int x; float y; bool flag; x = 5; y = 3.14; flag = x > y; flag")
		assert.Nil(t, err)
		...
		// 测试另一种混合类型情况
		compiledFunc2, err := Compile[*MockKv]("int count; float rate; count = 10; rate = 0.8; count * rate")
		assert.Nil(t, err)
	...
	t.Run("深层嵌套表达式", func(t *testing.T) {
		compiledFunc, err := Compile[*MockKv]("int a, b, c, d; ((a + b) * (c - d)) / ((a - b) + (c + d))")
		assert.Nil(t, err)
	...
	t.Run("布尔短路求值", func(t *testing.T) {
		// 测试 && 短路 - 修改为不使用内联赋值
		compiledFunc, err := Compile[*MockKv]("bool flag1; int x; flag1 = false; x = 0; flag1 && x > 0")
		assert.Nil(t, err)
		...
		// 测试 || 短路
		compiledFunc2, err := Compile[*MockKv]("bool flag1; int y; flag1 = true; y = 0; flag1 || y > 0")
		assert.Nil(t, err)
		...
		// 测试更复杂的短路情况
		compiledFunc3, err := Compile[*MockKv]("bool a, b; int count; a = false; b = true; count = 10; a && b && count > 5")
		assert.Nil(t, err)
	...
	// 测试数学函数组合
	t.Run("数学函数组合", func(t *testing.T) {
		// 测试复杂的幂运算组合
		compiledFunc, err := Compile[*MockKv]("int base, exp1, exp2; float result; base = 2; exp1 = 3; exp2 = 2; result = base ^ exp1 ^ exp2; result > 250")
		assert.Nil(t, err)
	...
	t.Run("边界条件测试", func(t *testing.T) {
		// 测试除零保护（应该在编译时或运行时检测）
		compiledFunc, err := Compile[*MockKv]("int x, y; x = 10; y = 0; x != 0 && y == 0")
		assert.Nil(t, err)
		...
		// 测试很大的数值
		compiledFunc2, err := Compile[*MockKv]("int large; large = 999999; large > 500000")
		assert.Nil(t, err)
	...
	t.Run("游戏逻辑场景", func(t *testing.T) {
		// 模拟游戏中的技能冷却和资源消耗判断
		compiledFunc, err := Compile[*MockKv](`
	...
	// 测试字符串式的复杂表达式
	t.Run("复杂字符串表达式", func(t *testing.T) {
		// 测试复杂的数学表达式
		compiledFunc, err := Compile[*MockKv]("float x, y, z, result; x = 1.5; y = 2.5; z = 3.5; result = (x + y) * z - (x * y) / z + x ^ 2 - y ^ 2")
		assert.Nil(t, err)
	...
	t.Run("除零错误", func(t *testing.T) {
		...
		// 取模除零
		compiledFunc2, err := Compile[*MockKv]("int x, y; x % y")
		assert.Nil(t, err)
...
func TestCompileErrors(t *testing.T) {
	...
	t.Run("复杂类型不匹配", func(t *testing.T) {
		// 测试布尔值与数字进行算术运算
		_, err := Compile[*MockKv]("bool flag; int x; flag + x")
		assert.NotNil(t, err)

		// 测试浮点数与布尔值比较
		_, err = Compile[*MockKv]("float x; bool flag; x == flag")
		assert.NotNil(t, err)

		// 测试对布尔值进行算术操作
		_, err = Compile[*MockKv]("bool a, b; a * b")
		assert.NotNil(t, err)
	})
	...
	t.Run("不支持的运算符组合", func(t *testing.T) {
		// 测试对字符串进行操作（不支持的类型）
		_, err := Compile[*MockKv]("string x")
		assert.NotNil(t, err)

		// 测试不完整的三目运算符
		_, err = Compile[*MockKv]("int x; x > 0 ? 1")
		assert.NotNil(t, err)

		// 测试括号不匹配
		_, err = Compile[*MockKv]("int x; (x + 1")
		assert.NotNil(t, err)
	})
	...
	t.Run("数值格式错误", func(t *testing.T) {
		// 测试非法的数字格式
		_, err := Compile[*MockKv]("int x; x = 3.14.159")
		assert.NotNil(t, err)

		// 测试空表达式
		_, err = Compile[*MockKv]("")
		assert.NotNil(t, err)

		// 测试只有类型声明没有表达式
		compiledFunc, err := Compile[*MockKv]("int x, y")
	...
	t.Run("逻辑运算类型约束", func(t *testing.T) {
		// 测试对浮点数使用逻辑非
		_, err := Compile[*MockKv]("float x; !x")
		assert.NotNil(t, err)

		// 测试浮点数的逻辑运算
		_, err = Compile[*MockKv]("float x, y; x && y")
		assert.NotNil(t, err)

		// 测试对布尔值使用算术运算符
		_, err = Compile[*MockKv]("bool x; +x")
		assert.NotNil(t, err)
	})
