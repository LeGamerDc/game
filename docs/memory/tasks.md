...
- [x] tag.Query 编译态优化：新增 `NewQuery(db, all, none, some)` 构造路径，在构造期完成层级归一化、冲突检测与冗余消除；`Match` 改为基于单 slice + boundary + kind mask 的运行时分派；补充 hierarchy/invalid/impossible/some-satisfied-by-all 测试并同步 README (2026-04-03)
