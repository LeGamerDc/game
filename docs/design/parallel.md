...
*当前状态：WorldView 已提供 `Version()` 和 `Round()` 观测接口，但版本化空间索引尚未实现。*

## 最小落地版本（已完成）

Scheduler 核心已实现（`sched/` 包），包括并发/串行双模式、timer wheel、block-based 分组、LPT 负载均衡、WatchState 机制等。详见 `docs/design/scheduler.md`。
