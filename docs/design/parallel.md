...
- 只读 world snapshot（类型为 `W`，由具体 World 类型决定可查询能力）
- Emit: 向目标 ref 发送 signal（用于通知后续逻辑）
- WriteStage: 提交当前 owner 的阶段稳定状态，例如刷新 framework 层 WatchState/订阅摘要

WatchState 不再是 scheduler runtime 原语。需要 `WatchOf(ref)` 这类查询时，由 framework 在 `StagedState` 上构建双缓冲/版本化视图，并把查询方法放到具体 `W` 类型上。

Apply 不应:

- 再次发 effect（否则 Apply 会退化成第二套脚本执行环境）
- 直接写其他 Logic 的状态
- 写其他 Logic 的 staged state（WriteStage 没有 ref 参数）
...
3. **StagedState 可承载发射端过滤**：WatchState 可作为 framework 层 staged state，用于实现发射端过滤。
...
- 框架级 pub-sub: Logic 订阅某类事件，框架自动做 fan-out 分发（StagedState 中的 WatchBits 可作为 pub-sub 的基础构件）
- 同 tick 加急投递: 某类 signal 需要比其他 signal 更早到达
- Replay/审计工具需要区分因果链

当前这些场景都可以通过 Logic 自己查订阅者 Emit、signal payload 携带 causal_id 等方式解决。StagedState 提供基础的阶段稳定视图提交机制，更高级的 pub-sub 可在此基础上构建。作为扩展点保留。
...
## LogicMeta: 声明式元数据（设计保留）

最初设想每个 Logic 实现带一组声明式元数据（max_effects_per_activation、priority、cost_hint 等），供调度器和检测工具使用。

当前状态：全局调度参数已通过 `ScheduleMeta` 统一管理（阈值、并发度、superstep 上限等），不需要 Logic 接口暴露 `Meta()` 方法。Per-logic 的预算控制和调度提示作为设计保留，待实际需求出现后再评估。
...
系统接受同一 round 内 effect 任意顺序。但"顺序无关"的含义是**容忍性**而非数学严格交换律——不同顺序可能产出不同中间结果，但对玩家而言都是可接受的。

因此 shuffle 测试（对同一组 effect 随机打乱验证结果一致）不适用：不同顺序本就可能产出不同的合法结果。无序安全的保障主要依靠设计审查和适配分类（参见 `docs/design/adaptation_guide.md` 中的 D 分类）。
...
*当前状态：Scheduler 接口已通过 ThinkCtx/CommitCtx 分层强制执行此约束。*

### 2. 调度风险

如果 current tick 允许无限追加 next-round think，很容易出现某些 tick 极端膨胀。

*当前状态：已通过 MaxSupersteps（并发）和 cascade depth（串行）实现防护。*

### 3. 热点风险

如果大量 effect 都集中到少数 boss/tower/objective 上，并行度会在 Apply 阶段塌缩到热点 owner。

*当前状态：Apply 阶段 LPT 负载均衡可部分缓解，但热点 owner 仍是潜在瓶颈，待 benchmark 验证。*

### 4. 快照风险

空间查询、可见性、附近目标列表这类索引如果没有版本化语义，Think 看到的世界会不一致。

*当前状态：`World` 已提供 `Version()` 和 `Round()` 观测接口，但版本化空间索引尚未实现。*

## 最小落地版本（已完成）

Scheduler 核心已实现（`sched/` 包），包括并发/串行双模式、timer wheel、block-based 分组、LPT 负载均衡、StagedState 机制等。详见 `docs/design/scheduler.md`。

下一步验证方向是在 Scheduler 之上实现 game ability system（技能/buff/伤害结算），作为端到端的最小闭环验证。
...
6. Effect 代数组合（框架级预合并）已确认不做；signal 语义分类作为未来可选扩展点保留
