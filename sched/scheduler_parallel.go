...
func (sc *Scheduler[W, S, E, L]) thinkWorker(threadId int, world W, includeTimers bool) {
	...
	ctx := &ThinkCtx[W, S, E]{
		...
		WriteStage: func(kind StageKind, state StagedState) {
			sc.stageCollectors[threadId].Put(stageKey{ref: thinkRef, kind: kind}, state)
		},
	}
...
func (sc *Scheduler[W, S, E, L]) applyWorker(threadId int, world W) {
	...
	ctx := &CommitCtx[W, S]{
		...
		Emit:  sc.emitClosure(threadId),
		WriteStage: func(kind StageKind, state StagedState) {
			sc.stageCollectors[threadId].Put(stageKey{ref: applyRef, kind: kind}, state)
		},
	}
